...
@CrossOrigin(origins = "http://localhost:4200") // ✅ Corrección: sin allowedMethods
public class LoteController {
    ...
    public ResponseEntity<MensajeResponse> ajustarStock(@PathVariable Integer id,
        ...
        if (request.getTipoMovimiento() != MovimientoStock.TipoMovimiento.ajuste
                && request.getTipoMovimiento() != MovimientoStock.TipoMovimiento.merma) {
            throw new ParametroInvalidoException(
                    "Este endpoint solo admite ajustes de tipo 'ajuste' (aumenta stock) o 'merma' (reduce stock). "
                    + "Para compras registra un nuevo lote, para ventas usa el módulo de Ventas.");
        }

        int delta = request.getTipoMovimiento() == MovimientoStock.TipoMovimiento.merma
                ? -request.getCantidad()
                : request.getCantidad();

        loteService.ajustarStock(
                id,
                delta,
                request.getUsuarioId(),
                request.getTipoMovimiento().name(),
                request.getObservacion()
        );

        String mensaje = request.getTipoMovimiento() == MovimientoStock.TipoMovimiento.merma
                ? "Merma registrada correctamente. Stock actualizado."
